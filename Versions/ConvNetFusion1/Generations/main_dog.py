...
dog_images = [img for img, label in cifar10 if label == 5]

image_size = dog_images[0].numel()
bird_images_tensor = torch.stack([img.flatten() for img in dog_images])
device = torch.device("cpu")
y_train = bird_images_tensor.to(device)
X_train = torch.randn((len(dog_images), image_size)).to(device)
