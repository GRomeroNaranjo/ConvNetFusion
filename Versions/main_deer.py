...
deer_images = [img for img, label in cifar10 if label == 4]

image_size = deer_images[0].numel()
bird_images_tensor = torch.stack([img.flatten() for img in deer_images])
device = torch.device("cpu")
y_train = bird_images_tensor.to(device)
X_train = torch.randn((len(deer_images), image_size)).to(device)
