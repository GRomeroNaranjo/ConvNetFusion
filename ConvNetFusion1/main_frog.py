...
frog_images = [img for img, label in cifar10 if label == 5]

image_size = frog_images[0].numel()
bird_images_tensor = torch.stack([img.flatten() for img in frog_images])
device = torch.device("cpu")
y_train = bird_images_tensor.to(device)
X_train = torch.randn((len(frog_images), image_size)).to(device)
